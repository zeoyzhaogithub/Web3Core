...
跨域问题是由浏览器的**同源策略**引起的一种安全限制。同源策略是浏览器的一个核心安全功能，它规定：**当一个源的文档或脚本，试图去请求另一个源的资源时，浏览器会默认阻止这个请求**。

这里的“源”，指的是一个完整的URL地址的身份，只有当**协议**（Protocol）， **域名**（Domain），**端口**（Port）三者都完全相同时，才被认为是**同源**。只要其中有任何一项不同，就被认为是**跨域**
...
- 前端应用（如 `http://localhost:3000`）调用后端API（如 `https://api.company.com`）
- 主站（如 `https://www.site.com`）调用子域名的服务（如 `https://api.site.com`）
- 使用不同协议访问资源（如从 `http` 站点请求 `https` 资源）

### 18、同源策略？

同源策略 是浏览器实施的一种核心安全机制。它规定了 从一个源加载的文档或脚本，如何与来自另一个源的资源进行交互。

同源策略的存在目的，主要是为了**保护用户的信息安全和隐私**，防止恶意网站窃取数据。

- **经典攻击场景**：假设你没有登出网上银行（`https://my-bank.com`），然后访问了一个恶意网站（`http://evil-site.com`）。如果没有同源策略，`evil-site.com` 的恶意脚本就可以向 `my-bank.com` 发起请求（这个请求会自动携带你的银行登录Cookie），读取账户余额、并进行转账操作。同源策略阻止了这种行为。

一个“源”由三个部分唯一确定：**协议**，**域名**，**端口**。只有当这三个部分都**完全一致**时，才被认为是同源。
...
- **限制**：默认情况下，使用 `XMLHttpRequest` 或 `Fetch API` 发起的跨域请求会被浏览器阻止，你无法读取返回的内容。
- **例外**：**请求实际上已经发出并且服务器也响应了**，只是浏览器出于安全考虑，拦截了响应，不将其交给前端脚本。你可以在浏览器的 Network 面板中看到这个请求。

**2. DOM 查询与操作**

- **限制**：如果页面中通过 `<iframe>` 嵌入了一个不同源的页面，那么父页面的 JavaScript 无法读取或修改子iframe中的DOM，反之亦然。
- **示例**：你的网站 `a.com` 无法通过 `iframe.contentWindow.document` 来获取嵌入的 `b.com` 页面的内容。

**3. 浏览器存储数据访问**

- **限制**：无法通过 JavaScript 读取不同源网站的 Cookie、LocalStorage、SessionStorage 和 IndexedDB。

**不受同源策略限制的标签**

有一些HTML标签允许跨域加载资源，这是为了网页能正常聚合多方内容。但这些标签**通常不允许**用JavaScript读取其内容。

- `<img src="...">`
- `<link rel="stylesheet" href="...">`
- `<script src="...">`
- `<video>` / `<audio>`
- `<iframe src="...">` (可以嵌入，但不能操作DOM)

**注意**：`<script>` 标签的跨域特性正是 **JSONP** 这种古老跨域方案的基础。

### 19、有哪些方法可以解决跨域？

跨域是浏览器带来的限制，解决方案也主要围绕浏览器和服务端来设计。

1. CORS - 跨域资源共享（**最主流方案**）

这是W3C标准，也是**最推荐、最常用**的解决方案。

**工作原理：**

- 浏览器在发现跨域请求时，会自动在请求头中添加一个 `Origin` 字段，表明请求来自哪个源
- 服务器检查这个 `Origin`，如果同意请求，就在响应头中包含特定的CORS头部
- 浏览器看到这些响应头后，就会允许本次跨域请求
...
- **简单请求**：GET/POST请求且使用简单Content-Type，浏览器直接发出
- **预检请求**：对于复杂请求，浏览器会先发送OPTIONS请求询问服务器是否允许

2. 代理服务器（开发环境常用）

**原理：** 因为同源策略是浏览器的限制，服务器之间没有这个限制。可以设置一个与前端同源的代理服务器，由它转发请求到目标服务器。

**应用：** 在Vue/React项目中，可以通过webpack-dev-server或专门的代理工具配置：
